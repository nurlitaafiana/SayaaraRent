        ...
        <div class="card-body p-4">
            <form action="{{ route('customer.rental.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <select name="kendaraan_id" id="kendaraan_id"
                        ...
                        @foreach($kendaraans as $mobil)
                            <option value="{{ $mobil->id }}"
                                {{ (old('kendaraan_id', $kendaraan?->id) == $mobil->id) ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    @error('upload_ktp') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    @error('upload_sim') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
...
@push('scripts')
<script>
    // Disable tanggal yang sudah dipesan
    ...
    function setMinDate(inputId, baseDate) {
        document.getElementById(inputId).min = baseDate;
    }

    // Validasi saat user pilih tanggal
    ...
    // Ganti kendaraan → reload halaman dengan kendaraan baru
